  ...
  <nav
    ...
    <template
      ...
      <router-link
        ...
        @click="emit('projectSelected')"><span>{{ project.title }}</span>
      </router-link>
    </template>
  </nav>

  <div v-else>
    🤔 Hmmm... aucun projet ici...
  </div>

  <!-- Show closed switch -->
  <div
    v-if="projectStore.projects.filter(project => project.closed).length"
    class="form-check form-switch mx-3">
    <input
    ...
    <label
      ...
      for="show-closed-projects">Afficher les projets terminés</label>
  </div>
</template>
